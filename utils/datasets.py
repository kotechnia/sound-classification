...
class ESC50(td.Dataset):
    ...
    def __init__(self,
        ...
        meta = self.load_meta(os.path.join(root, 'meta', 'meta.csv'))
...
class ESC10(ESC50):
    ...
    def __init__(self,
        ...
        self.classes = {
            ...
            for new_target, old_target
            in enumerate({item['target'] for item in self.data.values()})
        }

    @staticmethod
    def load_meta(path_to_csv: str) -> pd.DataFrame:
        meta = ESC50.load_meta(path_to_csv)
        meta.drop(index=meta[~meta['esc10']].index, inplace=True)

        return meta

    def __getitem__(self, index: int) -> Tuple[np.ndarray, int]:
        audio, target = super(ESC10, self).__getitem__(index)
...
class UrbanSound8K(td.Dataset):
    ...
    def load_data(self):
        ...
        warnings.filterwarnings('ignore')
        with mp.Pool(processes=mp.cpu_count()) as pool:
            chunksize = int(np.ceil(len(meta) / pool._processes)) or 1
...
class NIA2022(td.Dataset):
    def __init__(self,
        ...
        if meta_path is not None:
            ...
            meta = self.load_meta(os.path.join(root, 'dataset.csv'))
        ...
        if self.train:
            self.folds_to_load = ['train']
        else:
            self.folds_to_load = ['val']
        

        self.data = dict()
        #self.load_data(meta, os.path.join(root, 'audio'))
        self.load_data(meta, os.path.join(root))
        self.indices = list(self.data.keys())
        self.target_transform = target_transform
    ...
    def load_data(self, meta: pd.DataFrame, base_path: str):
        ...
        for idx, filepath, sample_rate in tqdm.tqdm(items_to_load, ascii=True):
            #_, _, wav = self._load_worker(idx, filepath, sample_rate) 
            row = meta.loc[idx]
            self.data[idx] = {
                'audio': filepath,
                'sample_rate': sample_rate,
    ...
    def __getitem__(self, index: int) -> Tuple[np.ndarray, int]:
        ...
        _, sr = librosa.load(self.data[self.indices[index]]['audio'], self.data[self.indices[index]]['sample_rate'])

        _, _, wav =  self._load_worker(0, self.data[self.indices[index]]['audio'], self.data[self.indices[index]]['sample_rate'])

        audio: np.ndarray = wav
        target: int = self.data[self.indices[index]]['target']
