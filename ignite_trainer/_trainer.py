...
def run(experiment_name: str,
    ...
    with _utils.tqdm_stdout(orig_stdout) as orig_stdout:
        ...
        prog_bar_epochs = tqdm.tqdm(total=epochs, desc='Epochs', file=orig_stdout, dynamic_ncols=True, unit='epoch',ascii=True)
        prog_bar_iters = tqdm.tqdm(desc='Batches', file=orig_stdout, dynamic_ncols=True, ascii=True)
        ...
        def log_training(engine: ieng.Engine):
            ...
            if num_iter % log_interval == 0 or num_iter == len(train_loader) or early_stop:
                tqdm.tqdm.write(
                    '{} Epoch[{}] Iteration[{}/{}] Loss: {:.4f}'.format(current_time(),
        ...
        def log_validation(engine: ieng.Engine,
            ...
            tqdm.tqdm.write('{} {} results - {}'.format(current_time(), run_type, '; '.join(tqdm_info)))
...
def main():
    with _utils.tqdm_stdout() as orig_stdout:
        ...
        parser.add_argument('-c', '--config', type=str, default='protocols/nia2022/nia2022.json')
